...
from optims import get_optimizer, LinearWarmupCosineLRScheduler, get_optimizer_stage2
from models.utils import decode_time_answer_v3
...
class Runner:
    def __init__(self, cfg, model, datasets, job_id):
        ...
        if not self.evaluate_only:
            ...
            self.test_save = self.config.config.datasets.test_save_path
        ...
        if not self.evaluate_only:
            ...
            if self.config.config.model.tune_added_tokens == "stage2":
                main_lr = self.config.config.run.optims.init_lr
                adapter_lr = self.config.config.run.optims.get("adapter_lr", main_lr / 10.0)
                logging.info(f"Setting up optimizer with Main LR: {main_lr} and Adapter LR: {adapter_lr}")
                lora_params = []
                adapter_params = []
                ...
                for name, param in model_no_ddp.named_parameters():
                    if not param.requires_grad:
                        continue
                    if "lora_" in name and "llama_model" in name:
                        lora_params.append(param)
                    # Assign Q-Former, Proj layer, and other adapter-like parameters to the low learning rate group
                    else:
                        adapter_params.append(param)
                
                if not lora_params:
                    logging.warning("No LoRA parameters found for the main learning rate group. All trainable params will use adapter_lr.")
                    # If no LoRA params, put all in one group to avoid errors
                    optimizer_grouped_parameters = [{"params": adapter_params, "lr": adapter_lr}]
                else:
                    optimizer_grouped_parameters = [
                        {
                            "params": lora_params,
                            "lr": main_lr,
                            "weight_decay": self.config.config.run.optims.get("weight_decay", 0.05),
                        },
                        {
                ...
                self.optimizer = get_optimizer_stage2(optimizer_grouped_parameters, self.config.config.run.optims)
            else:
            ...
            # resume
            self.resume_epoch =self.config.config.run.resume_epoch
            if self.resume_epoch is not None:
                checkpoint_path = os.path.join(self.output_dir, f"checkpoint_{self.resume_epoch}.pth")
                if os.path.exists(checkpoint_path):
                    logging.info(f"resume checkpoint from {checkpoint_path}")
                    self.start_epoch = self.load_checkpoint(checkpoint_path) + 1
                else:
                    logging.warning(f"Checkpoint at epoch {self.resume_epoch} not found. Starting from scratch.")
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        model_no_ddp = self.unwrap_dist_model(self.model)
        model_no_ddp.load_state_dict(checkpoint["model"], strict=True)
        self.optimizer.load_state_dict(checkpoint["optimizer"])
        if self.use_amp and "scaler" in checkpoint and checkpoint["scaler"] is not None:
            self.scaler.load_state_dict(checkpoint["scaler"])

        if hasattr(self, 'scheduler'):
            self.scheduler.step(loaded_epoch + 1)
            logging.info(f"Scheduler state fast-forwarded to epoch {loaded_epoch + 1}.")

        loaded_epoch = checkpoint["epoch"]
        logging.info(f"Checkpoint loaded from {checkpoint_path}. Last completed epoch was {loaded_epoch}")
    ...
    def valid_epoch(self, epoch, split, decode=False, save_json=False):
        ...
        for samples in metric_logger.log_every(dataloader, self.config.config.run.log_freq, header=header):
            ...
            res = {
                "id": samples["id_audio"][0],
                "audio": samples["id"][0],
                "answer": decode_time_answer_v3(samples["text"][0]),
                "loss": loss.item(),
            ...
            if decode:
                if model.prompt_dict:
                    if self.test_prompt_dict is None:
                        ...
                        if "Q" in samples:
                            prompts = [self.prompt_template.format(p) for p in samples["Q"]]
                ...
                res["model_prediction"] = (text[0])
                res["question"] = prompts[0].replace('<Speech><SpeechHere></Speech> ','')
            results.append(res)
        ...
        for item in results:
            item_loss = item["loss"]
            item_n_sample = len(item["audio"].split('/')[-1])
        ...
        if save_json:
            for res in results:
                ...
                del res["acc"]
                del res["total"]
            self.save_result(results, self.output_dir, self.test_save)
        return ret
    ...
    def save_result(self, result, result_dir, filename):
        ...
        # )
        final_result_file = filename #os.path.join(result_dir, "%s" % filename)
    ...
    def infer(self):
        ...
        total_time = time.time() - start_time
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        logging.info("Inference time {}".format(total_time_str))
            ...
            for k, v in state_dict.items():
                ...
                if k.startswith('llama_model.'):
                    if 'embed_tokens' in k or 'lm_head' in k:
                        filtered_state_dict[k] = v
                    if 'abs_time_position_embedding' in k:
                        filtered_state_dict[k] = v
                elif k.startswith('speech_encoder.') or k.startswith('beats.'):
